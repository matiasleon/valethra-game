...
- `apparition-actor.ts` construye el rig y aplica poses. Brazos articulados desde el hombro y arma sujeta a la mano; el desplazamiento visual nunca altera el origen de combate.
...
Se retiró del web el grafo no referenciado del puente (`bridge-scene`, `exploration`, `encounter-rules`, sus pruebas y retratos duplicados). Godot conserva el prototipo histórico de Grukh. Las criaturas del santuario siguen siendo apariciones: no se modificó su identidad ni el canon.
